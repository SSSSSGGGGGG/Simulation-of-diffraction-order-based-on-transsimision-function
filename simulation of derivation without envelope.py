...
expr = (-1)**n * a * (-sp.sin(sp.pi * m / 2) * sp.sin(sp.pi * phi / 2) + sp.cos(sp.pi * m / 2) * sp.cos(sp.pi * phi / 2)) * sp.sinc((m/2)) *sp.DiracDelta(u-(N/(k*delta)))

# Compute the summation for each solution of n

result_sub=expr.subs({k:8,N:5,m:5-8*n}) #!!!! change N and m at the same time.
